...
public class AudioManager : MonoBehaviour {
	...
	[Tooltip("Maximum number of AudioSource components on this gameObject")]
	[SerializeField]
	private int maxNbAudioSources = 15;
	private List<AudioClip> listOfMiscClips;
	private List<AudioSource> availableAudioSourceList = new List<AudioSource>();
	private List<AudioSource> playingAudioSourceList = new List<AudioSource>();
	...
	void Update() {
		for (int i = playingAudioSourceList.Count; i > 0;) {
			i--;
	...
	public AudioSource PlayClip(AudioClip clip) {
		return PlayClip(clip, false, 0.0f, 1.0f, 1.0f, 1.0f);
	}
	...
	public AudioSource PlayClip(AudioClip clip, bool loop) {
		return PlayClip(clip, loop, 0.0f, 1.0f, 1.0f, 1.0f);
	}
	...
	public AudioSource PlayClip(AudioClip clip, float fadeInLength) {
		return PlayClip(clip, false, fadeInLength, 1.0f, 1.0f, 1.0f);
	}

	/// <summary>
	/// Plays an AudioClip with a pitch between a specified minimum and maximum, returns the AudioSource used to play it
	/// </summary>
	public AudioSource PlayClip(AudioClip clip, float minPitch, float maxPitch) {
		return PlayClip(clip, false, 0.0f, 1.0f, minPitch, maxPitch);
	}

	/// <summary>
	/// Plays an AudioClip with a fade in, a pitch and a loop option, returns the AudioSource used to play the AudioClip
	/// </summary>
	public AudioSource PlayClip(AudioClip clip, bool loop, float fadeInLength, float desiredVolume, float minPitch, float maxPitch) {
		if (clip) {
